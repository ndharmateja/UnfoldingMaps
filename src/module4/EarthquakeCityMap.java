...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {		
		...
		size(1800, 1100, OPENGL);
		if (offline) {
		    map = new UnfoldingMap(this, 250, 50, 1500, 1000, new MBTilesMapProvider(mbTilesString));
		    earthquakesURL = "2.5_week.atom";  // The same feed, but saved August 7, 2015
		}
		else {
			AbstractMapProvider provider = new Microsoft.RoadProvider();
			//AbstractMapProvider provider = new GoogleTerrainProvider();
			map = new UnfoldingMap(this, 250, 50, 1500, 1000, provider);
	...
	private void printQuakes() 
	{
		...
		for (Marker countryMarker : countryMarkers) {
			Object country = countryMarker.getProperty("name");
			int countryCount = 0;
			for (Marker quakeMarker : quakeMarkers) {
				if (quakeMarker instanceof LandQuakeMarker) {
					if (country.equals(quakeMarker.getProperty("country"))) {
						countryCount += 1;
					}
				}
			}
